...
export function CommandSwapElement({ id, command }: Props): JSX.Element {
    ...
    useEffect(() => {
        staticRpc.unpackFromCell({
            ...
            const _token = tokenByAddress(data.to.toString())
            setToken(_token || { address: data.to.toString(), decimals: 0 })
